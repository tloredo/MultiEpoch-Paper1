...
# regexp for grabbing figure files from \plotone{PATH}:
pfig_re = re.compile(r'.*\plotone{(?P<fig>[\w,\-,+,.,/]*)}')
    
# regexp for grabbing input files:
...
def find_files(lines):
    ...
    for line in lines:
        ...
        results = pfig_re.findall(line)
        c = line.count('plotone')
        if c > 1:
            print '*** Multiple plotone figures on a line! ***'
        for result in results:
            figs.append(result)
